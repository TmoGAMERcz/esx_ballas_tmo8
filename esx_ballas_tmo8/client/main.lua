...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = _U('ballas_wear'), uniform = grade},
			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerEvent('esx_ballas:updateBlip')
...
function OpenArmoryMenu(station)
	...
	if Config.OxInventory then
		exports.ox_inventory:openInventory('stash', {id = 'society_ballas', owner = station})
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuff' then
						TriggerServerEvent('esx_ballas:handcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						TriggerServerEvent('esx_ballas:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_ballas:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_ballas:OutVehicle', GetPlayerServerId(closestPlayer))
	...
	ESX.TriggerServerCallback('esx_ballas:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('esx_ballas:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('esx_ballas:getFineList', function(fines)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_ballas', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('esx_ballas:getVehicleInfos', function(retrivedInfo)
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_ballas:getOtherPlayerData', function(playerData)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerServerEvent('esx_ballas:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('esx_ballas:buyWeapon', function(bought)
				if bought then
...
AddEventHandler('esx:setJob', function(job)
	...
	if job.name == 'ballas' then
		Wait(1000)
		TriggerServerEvent('esx_ballas:forceBlip')
	end
end)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ballas' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_ballas:hasEnteredEntityZone', function(entity)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ballas' and IsPedOnFoot(playerPed) then
...
RegisterNetEvent('esx_ballas:unrestrain')
AddEventHandler('esx_ballas:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('esx_ballas:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('esx_ballas:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'ballas' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'ballas_actions') then
			if not Config.EnableESXService then
...
RegisterNetEvent('esx_ballas:updateBlip')
AddEventHandler('esx_ballas:updateBlip', function()
...
if ESX.PlayerLoaded and ESX.PlayerData.job == 'ballas' then
